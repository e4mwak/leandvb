...
struct config {
  ...
  bool loop_input;
  bool live_input;     // Keep stepping on live stdin instead of exiting on fixpoint
...
static void run_scheduler(config &cfg, scheduler *sch) {
#ifdef _WIN32
  if ( cfg.live_input ) {
    unsigned long long prev_hash = 0;
    while ( 1 ) {
      sch->step();
      unsigned long long h = sch->hash();
      if ( h == prev_hash ) Sleep(1);
      prev_hash = h;
    }
  }
#endif
...
void usage(const char *name, FILE *f, int c, const char *info=NULL) {
  ...
  fprintf
    (f,
     ...
     "  --live-input   Keep waiting for live stdin instead of exiting\n"
...
int main(int argc, const char *argv[]) {
  ...
  for ( int i=1; i<argc; ++i ) {
	...
	usage(argv[0], stderr, 1, argv[i]);
      ...
      cfg.loop_input = true;
    else if ( ! strcmp(argv[i], "--live-input") )
      cfg.live_input = true;
